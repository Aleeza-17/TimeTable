...
class Coursecp extends StatefulWidget {
  String myData = ""; // Declare a final variable to store the data

  Coursecp({required this.myData});
  @override
  State<Coursecp> createState() => _CoursecpState(myData);
}

class _CoursecpState extends State<Coursecp> {
  String mydata1 = "";
  _CoursecpState(this.mydata1);
  ...
  Future<void> departmentdata() async {
    ...
    if (_selectedDept != 0) {
      print(_selectedDept);
      final results = await conn.query(
          'SELECT Courseid ,Course,creditHour,DepId,FacultyId,Batch FROM course WHERE DepId = $_selectedDept');
