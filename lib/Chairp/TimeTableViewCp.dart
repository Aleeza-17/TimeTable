...
class CPTimeTableView1 extends StatefulWidget {
String myData=""; // Declare a final variable to store the data

  CPTimeTableView1({required this.myData});
  @override
  State<CPTimeTableView1> createState() => _CPTimeTableView1State(myData);
}

class _CPTimeTableView1State extends State<CPTimeTableView1> {
  String mydata1="";
   _CPTimeTableView1State(this.mydata1);
...
Future<void> cancel_class() async {
    ...
    for (var row in results) {
      // Convert each row to a List<dynamic> and convert int to String
      cancel.add([row[0].toString(), row[1].toString(), row[2].toString(), row[3].toString(), row[4].toString(), row[5].toString(), row[6].toString()]);
    ...
    return Container(
      decoration: BoxDecoration(
        image: DecorationImage(
          image: AssetImage('images/WhatsApp Image 2023-06-24 at 6.07.56 AM.jpeg'), // Replace with your image path
          fit: BoxFit.cover,
        ),
      ),
      child: Scaffold(
              ...
              Expanded(
                child: SingleChildScrollView( // Wrap the ListView with SingleChildScrollView
                  child: Center(
                    ...
                    rows: _data.map((row) {
                      bool isCancelled = _cancel.any((cancelRow) => row[0] == cancelRow[3]);
                      Color rowColor = isCancelled ? Colors.red: Colors.transparent;
