...
class Reschedule extends StatefulWidget {
  ...
  Reschedule({required this.myData});
  @override
  State<Reschedule> createState() => _RescheduleState(myData);
}

class _RescheduleState extends State<Reschedule> {
  String mydata1 = "";
  _RescheduleState(this.mydata1);
  ...
  Future<void> departmentdata() async {
    ...
    final results = await conn
        .query('select DepId,Batch FROM  students  WHERE UserId  = $mydata1');
    ...
    if (_selectedDept != 0) {
      ...
      final results = await conn.query(
          'SELECT RescheduleId,StartTime,EndTime,Subject,date,Teacher,DepId,Batch,RoomNo FROM reschedule WHERE DepId = $_selectedDept && Batch= $batch');
